...
const ComexForm = ({ config, onChange, globalBanAll }) => {
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12">
            <div>
                <InputField 
                    label="Comex brokerage" 
                    name="brokerage" 
                    value={config.brokerage} 
                    onChange={(e) => onChange(e.target.name, e.target.value)} 
                />
                <InputField 
                    label="Maximum lots allowed per single trade of comex" 
                    name="maxLot" 
                    value={config.maxLot} 
                    onChange={(e) => onChange(e.target.name, e.target.value)} 
                />
                    <InputField 
                    label="Max Size All comex" 
                    name="maxSizeAll" 
                    value={config.maxSizeAll} 
                    onChange={(e) => onChange(e.target.name, e.target.value)} 
                />
                <InputField 
                    label="Holding Exposure/Margin comex" 
                    name="holdingMargin" 
            ...
            <div>
                ...
                />
                <InputField 
                    label="Minimum lots required per single trade of comex" 
                    name="minLot" 
                    value={config.minLot} 
                    onChange={(e) => onChange(e.target.name, e.target.value)} 
                />
                <InputField 
                    label="Maximum lots allowed per scrip of comex to be actively open at a time" 
                    name="maxLotScrip" 
                    value={config.maxLotScrip} 
                    onChange={(e) => onChange(e.target.name, e.target.value)} 
                />
                <InputField 
                    label="Intraday Exposure/Margin comex" 
                    name="intradayMargin" 
                ...
                />
                <InputField 
                    label="Orders to be away by % from current price comex" 
                    name="ordersAway" 
